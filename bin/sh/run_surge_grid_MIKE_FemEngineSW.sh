...
#--- OPTIONS ---
OPTIONS=$(getopt -o q:P:n:h:m --long many,norun,email,mem:,input:,help,norun,pe: -- "$@")
[ $? -eq 0 ] || { 
...
#---Create Single SGE Script ---
  ...
  echo >>$SGE_SH 
  echo >>$SGE_SH "module purge"
  echo >>$SGE_SH "module load mdtk surgetk"
  echo >>$SGE_SH "module load MIKE"
 
 
  #--- MAIN JOB SCRIPT ---
  
  echo >>$SGE_SH "cp -f \$PE_HOSTFILE ."
  #echo >>$SGE_SH "\$SURGETK_SCRIPTS/sort_hostfile.sh \$PE_HOSTFILE > hostfile.dat"
  echo >>$SGE_SH "\$MDTK_SCRIPTS/sort-hostfile-intel-mpi.sh \$PE_HOSTFILE > hostfile.dat"
  echo >>$SGE_SH "cat hostfile.dat"
  echo >>$SGE_SH "export OMP_NUM_THREADS=1"
